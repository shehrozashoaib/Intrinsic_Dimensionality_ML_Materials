...
# ALIGNN Fastfood intrinsic-dimension sweep for MATBENCH phonons (full dataset).
...
#   1. PHON_RUNS env var set  -> run exactly those entries; write per-run
...
CONFIG_ABS="${PROJECT_DIR}/configs/config_phonons.json"
TARGET_KEY="phonons"
ID_KEY="material_id"
WRAPPER="fastfood"
EPOCHS="${PHON_EPOCHS:-80}"           # full sweep default; smoke overrides this
BATCH_SIZE="${PHON_BATCH_SIZE:-}"      # empty -> use config_eform.json batch_size (64)
DATASET_DIR="${PHON_DATASET:-MP_json_phonons}"
if [[ "$DATASET_DIR" = /* ]]; then
  ROOT_ABS="$DATASET_DIR"
else
  ROOT_ABS="${ALIGNN_DIR}/${DATASET_DIR}"
fi
PRINT_PREFIX="[ALIGNN-KVRH]"
...
RESULT_ROOT="${PHON_RESULT_ROOT:-${PROJECT_DIR}/results_alignn_phonons_fastfood}"
SUMMARY_CSV="${RESULT_ROOT}/alignn_phonons_fastfood_summary.csv"
mkdir -p "$RESULT_ROOT"

# Flat run list. "id_dim:percent:model_seed:split_seed"
# Phonons: dims 40/60/80/100% x 10 seeds (model s, split s+1000). 40 runs.
RUNS=()
for spec in "0.2:20" "0.15:15" "0.1:10" "0.05:5" "0.02:2" "0.01:1"; do
  frac="${spec%%:*}"; pct="${spec##*:}"
  for s in 123 456 789 234 567 891 345 678 912 135; do
...
run_one() {
  ...
  local run_name="alignn_phonons_${WRAPPER}_dim${id_dim_percent}_modelseed${model_seed}_splitseed${split_seed}_epochs${EPOCHS}"
  ...
  append_metadata "$SUMMARY_CSV" "$metadata_json" "$log_file" "$pred_csv" "$history_json" \
    "matbench_phonons" "$TARGET_KEY" "$WRAPPER" "$id_dim" "$id_dim_percent" "$split_seed" "$model_seed" "$split_seed" \
...
if [[ -n "${PHON_RUNS:-}" ]]; then
  echo "$PRINT_PREFIX explicit selection: ${PHON_RUNS}"
  for item in $PHON_RUNS; do
