...
CONFIG_ABS="${PROJECT_DIR}/configs/config_log_kvrh.json"
TARGET_KEY="log_kvrh"
ID_KEY="material_id"
WRAPPER="fastfood"
EPOCHS="${KVRH_EPOCHS:-120}"           # full sweep default; smoke overrides this
...
RESULT_ROOT="${PROJECT_DIR}/results_alignn_log_kvrh_fastfood"
SUMMARY_CSV="${RESULT_ROOT}/alignn_log_kvrh_fastfood_summary.csv"
mkdir -p "$RESULT_ROOT"
...
run_one() {
  ...
  local run_name="alignn_log_kvrh_${WRAPPER}_dim${id_dim_percent}_modelseed${model_seed}_splitseed${split_seed}_epochs${EPOCHS}"
