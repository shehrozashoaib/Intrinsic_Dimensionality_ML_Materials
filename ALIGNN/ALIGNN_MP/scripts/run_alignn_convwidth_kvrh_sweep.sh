...
# ALIGNN Fastfood intrinsic-dimension sweep for MATBENCH log_kvrh (full dataset).
...
HIDDEN="${ALIGNN_HIDDEN:?set ALIGNN_HIDDEN=64|128|256}"
CONFIG_ABS="${PROJECT_DIR}/configs/config_log_kvrh_h${HIDDEN}.json"
TARGET_KEY="log_kvrh"
ID_KEY="material_id"
WRAPPER="fastfood"
...
DATASET_DIR="${KVRH_DATASET:-MP_json_log_kvrh}"
if [[ "$DATASET_DIR" = /* ]]; then
...
RESULT_ROOT="${PROJECT_DIR}/results_alignn_log_kvrh_convwidth_fastfood/h${HIDDEN}"
SUMMARY_CSV="${RESULT_ROOT}/alignn_log_kvrh_convwidth_h${HIDDEN}_summary.csv"
mkdir -p "$RESULT_ROOT"
...
for _d in "1.0:100" "0.8:80" "0.7:70" "0.65:65" "0.5:50" "0.45:45" "0.2:20" "0.1:10" "0.08:8" "0.05:5" "0.02:2" "0.01:1"; do
  _f="${_d%%:*}"; _p="${_d##*:}"
  for _s in 123 456 789; do RUNS+=("${_f}:${_p}:${_s}:$((_s+1000))"); done
done
...
run_one() {
  ...
  local run_name="alignn_log_kvrh_h${HIDDEN}_${WRAPPER}_dim${id_dim_percent}_modelseed${model_seed}_splitseed${split_seed}_epochs${EPOCHS}"
