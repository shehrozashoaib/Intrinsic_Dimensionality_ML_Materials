...
#   1. BG_RUNS env var set  -> run exactly those entries; write per-run
...
CONFIG_ABS="${PROJECT_DIR}/configs/config_bandgap.json"
TARGET_KEY="band_gap"
ID_KEY="material_id"
WRAPPER="fastfood"
EPOCHS="${BG_EPOCHS:-350}"           # full sweep default; smoke overrides this
BATCH_SIZE="${BG_BATCH_SIZE:-}"      # empty -> use config_eform.json batch_size (64)
DATASET_DIR="${BG_DATASET:-MP_json_bandgap}"
if [[ "$DATASET_DIR" = /* ]]; then
...
RESULT_ROOT="${PROJECT_DIR}/results_alignn_bandgap_custom_fastfood"
SUMMARY_CSV="${RESULT_ROOT}/alignn_bandgap_custom_fastfood_summary.csv"
mkdir -p "$RESULT_ROOT"
...
run_one() {
  ...
  local run_name="alignn_bandgap_${WRAPPER}_dim${id_dim_percent}_modelseed${model_seed}_splitseed${split_seed}_epochs${EPOCHS}"
  ...
  append_metadata "$SUMMARY_CSV" "$metadata_json" "$log_file" "$pred_csv" "$history_json" \
    "mp_bandgap" "$TARGET_KEY" "$WRAPPER" "$id_dim" "$id_dim_percent" "$split_seed" "$model_seed" "$split_seed" \
...
if [[ -n "${BG_RUNS:-}" ]]; then
  echo "$PRINT_PREFIX explicit selection: ${BG_RUNS}"
  for item in $BG_RUNS; do
