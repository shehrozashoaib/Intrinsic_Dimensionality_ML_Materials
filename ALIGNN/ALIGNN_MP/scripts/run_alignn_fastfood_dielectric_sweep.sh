...
# ALIGNN Fastfood intrinsic-dimension sweep for MATBENCH dielectric (full dataset).
...
#   1. DIEL_RUNS env var set  -> run exactly those entries; write per-run
...
CONFIG_ABS="${PROJECT_DIR}/configs/config_dielectric.json"
TARGET_KEY="dielectric"
ID_KEY="material_id"
WRAPPER="fastfood"
EPOCHS="${DIEL_EPOCHS:-150}"          # full sweep default; smoke overrides this
BATCH_SIZE="${DIEL_BATCH_SIZE:-}"      # empty -> use config_dielectric.json batch_size (64)
DATASET_DIR="${DIEL_DATASET:-MP_json_dielectric}"
if [[ "$DATASET_DIR" = /* ]]; then
  ROOT_ABS="$DATASET_DIR"
else
  ROOT_ABS="${ALIGNN_DIR}/${DATASET_DIR}"
fi
PRINT_PREFIX="[ALIGNN-DIELECTRIC]"
...
RESULT_ROOT="${DIEL_RESULT_ROOT:-${PROJECT_DIR}/results_alignn_dielectric_fastfood}"
SUMMARY_CSV="${RESULT_ROOT}/alignn_dielectric_fastfood_summary.csv"
mkdir -p "$RESULT_ROOT"

# Flat run list. "id_dim:percent:model_seed:split_seed"
# Dielectric: dims 1/2/5/10/15/20/40/60/80/100% x 5 seeds (model s, split s+1000). 50 runs.
RUNS=()
for spec in "1.0:100" "0.8:80" "0.6:60" "0.4:40" "0.2:20" "0.15:15" "0.1:10" "0.05:5" "0.02:2" "0.01:1"; do
  frac="${spec%%:*}"; pct="${spec##*:}"
  for s in 123 456 789 234 567; do
...
check_dataset() {
  if [[ ! -f "${ROOT_ABS}/id_prop.json" ]]; then
    ...
    echo "  conda run -n ${CONDA_ENV} python build_dielectric_idprop.py" >&2
    return 1
  fi
...
run_one() {
  ...
  local run_name="alignn_dielectric_${WRAPPER}_dim${id_dim_percent}_modelseed${model_seed}_splitseed${split_seed}_epochs${EPOCHS}"
  ...
  append_metadata "$SUMMARY_CSV" "$metadata_json" "$log_file" "$pred_csv" "$history_json" \
    "matbench_dielectric" "$TARGET_KEY" "$WRAPPER" "$id_dim" "$id_dim_percent" "$split_seed" "$model_seed" "$split_seed" \
...
if [[ -n "${DIEL_RUNS:-}" ]]; then
  echo "$PRINT_PREFIX explicit selection: ${DIEL_RUNS}"
  for item in $DIEL_RUNS; do
