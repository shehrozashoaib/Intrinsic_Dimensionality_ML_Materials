...
CACHED_DATA_DIR="${CGCNN_CACHED_DATA_DIR:-cached_matbench/tensors}"
BATCH_SIZE=512; N_CONV=1; ATOM_FEA_LEN=82; H_FEA_LEN=128; N_H=3
WRAPPER="fastfood"; OPTIM="Adam"; PRINT_FREQ=1000
# One-cycle arm (defaults preserve the historical MultiStepLR so prior results reproduce).
LR_SCHEDULE="${CGCNN_LR_SCHEDULE:-multistep}"
LR="${CGCNN_LR:-0.001}"
PCT_START="${CGCNN_PCT_START:-0.3}"
if [[ -n "${CGCNN_SEEDS:-}" ]]; then
  read -ra SEEDS <<< "$CGCNN_SEEDS"
else
  SEEDS=(123 456 789 234 567 891 345 678 912 135)
fi
# id-dim percentages -> fractions
if [[ -n "${CGCNN_DIM_SPECS:-}" ]]; then
  read -ra DIM_SPECS <<< "$CGCNN_DIM_SPECS"
else
  DIM_SPECS=("0.2:20" "0.15:15" "0.1:10" "0.05:5" "0.02:2" "0.01:1")
fi

RESULT_ROOT="${CGCNN_RESULT_ROOT:-results_matbench_${SHORT}_10seed_${WRAPPER}}"
...
run_one() {
  ...
  conda run -n "$CONDA_ENV" "$PYTHON" main.py \
    ...
    --data-seed "$seed" --random-seed "$seed" --print-freq "$PRINT_FREQ" \
    --lr-schedule "$LR_SCHEDULE" --lr "$LR" --pct-start "$PCT_START" 2>&1 | tee "$log"
  ...
  "$PYTHON" - "$SUMMARY_CSV" "$meta" "$log" "$pred_dst" "$TASK" "$WRAPPER" "$OPTIM" "$frac" "$pct" \
    ...
    "$status" "$ec" "$start" "$end" "$LR_SCHEDULE" "$LR" "$PCT_START" <<'PY'
import csv, json, math, re, sys
from pathlib import Path
(summary, meta, log, pred, task, wrapper, optim, frac, pct, seed, epochs, bs, nconv, atom,
 h, nh, out, dur, status, ec, start, end, lr_schedule, lr, pct_start) = sys.argv[1:]
...
# Collapse guard: std of this run's test predictions (a constant predictor sits near 0).
# CGCNN collapsed under one-cycle on matbench_dielectric, so this must be recorded.
try:
    _p = []
    with open(pred) as _f:
        for _r in csv.reader(_f):
            if len(_r) < 2:
                continue
            try:
                _p.append(float(_r[-1]))
            except ValueError:
                pass
    if len(_p) > 1:
        _mu = sum(_p) / len(_p)
        row["pred_spread"] = (sum((x - _mu) ** 2 for x in _p) / len(_p)) ** 0.5
except Exception:
    pass
