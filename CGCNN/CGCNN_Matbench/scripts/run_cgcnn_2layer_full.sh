#!/usr/bin/env bash
# CGCNN "doubled" model (n_conv=2) at FULL parameter space only (id_dim=1.0, no sweep).
...
SUMMARY_CSV="${RESULT_ROOT}/cgcnn_2layer_${TASK}_fastfood_summary.csv"
mkdir -p "$RESULT_ROOT"
...
run_one() {
  local seed="$1"
  local exp="cgcnn_2layer_${TASK}_${WRAPPER}_dim100_dataseed${seed}_modelseed${seed}"
  ...
  echo "[START] $exp seed=$seed $start"
  "$PYTHON" main.py \
    ...
    --subspace-method "$WRAPPER" --id-dim 1.0 \
  ...
  "$PYTHON" - "$SUMMARY_CSV" "$meta" "$log" "$pred_dst" "$MB_TASK" "$WRAPPER" "$OPTIM" "$seed" \
    "$EPOCHS" "$BATCH_SIZE" "$N_CONV" "$ATOM_FEA_LEN" "$H_FEA_LEN" "$N_H" "$out" "$dur" \
    "$status" "$ec" "$start" "$end" <<'PY'
import csv, json, math, re, sys
from pathlib import Path
(summary, meta, log, pred, task, wrapper, optim, seed, epochs, bs, nconv, atom,
...
row = dict(task=task, wrapper=wrapper, optim=optim, id_dim_frac=1.0, id_dim_percent=100,
