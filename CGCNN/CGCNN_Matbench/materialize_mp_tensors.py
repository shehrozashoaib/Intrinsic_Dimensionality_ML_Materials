...
def load_records(source, input_path, limit=0):
    ...
    if source == "mp21_jsonl":
        ...
        for r in rows:
            ...
            if limit and len(sdicts) >= limit:
                break
    elif source == "is_metal_idprop":
        # Binary classification: id_prop.json records {material_id, is_metal 0/1, atoms}.
        # Label stored as float 0.0/1.0; main.py casts to long() for NLLLoss.
        from jarvis.core.atoms import Atoms
        with open(input_path) as f:
            rows = json.load(f)
        for r in rows:
            if r.get("is_metal") is None or r.get("atoms") is None:
                continue
            sdicts.append(Atoms.from_dict(r["atoms"]).pymatgen_converter().as_dict())
            targets.append(float(int(r["is_metal"])))
            if limit and len(sdicts) >= limit:
                break
    else:
...
def main():
    ...
    p.add_argument("--source", required=True, choices=["eform_idprop", "mp21_jsonl", "is_metal_idprop"])
    ...
    p.add_argument("--graph-cache", default="",
                    help="Path to a built-graph cache (torch .pt of the `samples` list). "
                         "If it exists, graph construction is skipped entirely and only the "
                         "per-seed reshuffle runs. If it doesn't exist, it's written after "
                         "building so future --seeds-only runs are fast.")
    args = p.parse_args()
    ...
    if cache_path and cache_path.exists():
        ...
        N = len(samples)
        print(f"Loaded {N} cached graphs from {cache_path} in {time.time()-tc:.1f}s "
              f"(skipped load_records + build_one)", flush=True)
    else:
        ...
        if cache_path:
            ...
            print(f"Wrote graph cache -> {cache_path} in {time.time()-tw:.1f}s", flush=True)
