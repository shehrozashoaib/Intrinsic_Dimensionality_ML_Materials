...
parser = argparse.ArgumentParser("DimeNet++ matbench dielectric wrapper-v3 runner")
...
parser.add_argument("--cache_dir", type=str, default="./cached_tensors_dimenetpp_dielectric/splitseed1123")
parser.add_argument("--cache_file", type=str, default="dimenetpp_dielectric_cached_tensors.pkl")
parser.add_argument("--scaler_file", type=str, default="scaler_dielectric.pkl")
parser.add_argument("--out_dir", type=str, default="runs_dimenetpp_dielectric_v3_fastfood")
parser.add_argument("--lr", type=float, default=0.001, help="Optimizer learning rate (max_lr for onecycle).")
...
parser.add_argument("--lr_schedule", type=str, default="none", choices=["none", "onecycle", "cosine"],
                    help="LR schedule; onecycle anneals to ~0 so seeds converge.")
parser.add_argument("--pct_start", type=float, default=0.3, help="OneCycle: fraction of training spent rising.")
parser.add_argument("--restore_best", action="store_true",
                    help="Reload the best-val_loss checkpoint before predicting (otherwise the "
                         "FINAL-epoch, typically overfit, model is scored).")
parser.add_argument("--patience", type=int, default=0, help="EarlyStopping patience on val_loss (0=off).")
...
cfg = hyper_dielectric["model"]["config"]
...
hyper_dielectric["model"]["config"] = cfg
...
base_model = make_crystal_model(**hyper_dielectric["model"]["config"])

if args.train_mode == "base":
    run_model = base_model
    print("\nRunning BASE DimeNet++ dielectric model\n")
else:
    ...
    print("\nRunning WRAPPER-V3 DimeNet++ dielectric model\n")
...
if args.lr_schedule != "none":
    ...
    def _lr_at(epoch):
        ...
        if t < args.pct_start:
            return _init_lr + (_max_lr - _init_lr) * (t / args.pct_start)
        p = (t - args.pct_start) / (1 - args.pct_start)
    ...
    print(f"[lr_schedule] {args.lr_schedule}: init={_init_lr:.3e} max={_max_lr:.3e} "
          f"min={_min_lr:.3e} pct_start={args.pct_start}")
else:
...
# collapsed-predictor check: a constant-output model has ~0 spread; a real one is much larger
_spread = float(np.std(np.asarray(pred_test).reshape(-1)))
print(f"[pred_spread] {_spread:.6f}")
