...
NUM_BLOCKS=1   # FIXED: one interaction block; we vary conv WIDTH only
INT_EMB_SIZE="${INT_EMB_SIZE:-64}"   # conv (interaction) width: 64=1x, 128=2x, 256=4x
CLIPNORM="${DIMENET_CLIPNORM:-0}"    # 0 = no grad-norm clipping (set by smoke verdict)
WRAPPER="fastfood"
CACHE_ROOT="${DIMENET_CACHE_ROOT:-${PROJECT_DIR}/cached_tensors_dimenetpp_kvrh}"
RESULT_ROOT="${DIMENET_RESULT_ROOT:-${PROJECT_DIR}/results_dimenetpp_log_kvrh_convwidth_fastfood}"
SUMMARY_CSV="${RESULT_ROOT}/dimenetpp_log_kvrh_convwidth_ies${INT_EMB_SIZE}_summary.csv"
PRINT_PREFIX="[DIMENET-KVRH-ies${INT_EMB_SIZE}]"
...
run_one() {
  ...
  local run_name="dimenetpp_log_kvrh_${WRAPPER}_ies${INT_EMB_SIZE}_dim${id_dim_percent}_modelseed${model_seed}_splitseed${split_seed}_epochs${EPOCHS}"
