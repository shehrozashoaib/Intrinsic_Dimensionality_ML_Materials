...
# for matbench log_kvrh.
#
# One job = one partition: full 12-dim x SEEDS grid. Per-partition summary CSV avoids races
# between concurrently running partition jobs.
#
# Env vars:
#   DIMENET_PART       partition name, e.g. kvrh_quarter_1   (REQUIRED)
...
#
# Usage:
#   DIMENET_PART=kvrh_quarter_1 bash scripts/run_dimenetpp_kvrh_partition_sweep.sh
set -uo pipefail
...
PART="${DIMENET_PART:-}"
if [[ -z "$PART" ]]; then
  echo "ERROR: DIMENET_PART (e.g. kvrh_quarter_1) is required." >&2
  exit 1
fi

TASK="kvrh"              # short name used in file/dir names
...
RESULT_ROOT="${DIMENET_RESULT_ROOT:-${PROJECT_DIR}/results_dimenetpp_log_kvrh_partitions_fastfood}"
SUMMARY_CSV="${RESULT_ROOT}/dimenetpp_${TASK}_${PART}_summary.csv"
PRINT_PREFIX="[DIMENET-KVRH-${PART}]"
...
run_one() {
  ...
  append_metadata "$SUMMARY_CSV" "$metadata_json" "$log_file" \
    "$META_TASK" "$PART" "$NUM_BLOCKS" "$id_dim" "$id_dim_percent" "$model_seed" "$split_seed" \
...
echo "$PRINT_PREFIX task=${META_TASK} partition=${PART} num_blocks=${NUM_BLOCKS} int_emb_size=${INT_EMB_SIZE} epochs=${EPOCHS} clipnorm=${CLIPNORM} batch_size=${BATCH_SIZE}"
