#!/usr/bin/env bash
# DimeNet++ Fastfood intrinsic-dimension sweep for matbench dielectric (full dataset).
# Layer count (num_blocks) is fixed per job via NUM_BLOCKS env (so one job = one layer count,
# full 10-dim x 5-seed sweep). Per-num_blocks summary CSV avoids races between bundled jobs.
set -uo pipefail
...
EPOCHS="${DIMENET_EPOCHS:-150}"
BATCH_SIZE="${DIMENET_BATCH_SIZE:-64}"
GPU="${DIMENET_GPU:-0}"
NUM_BLOCKS="${NUM_BLOCKS:-1}"
WRAPPER="fastfood"
CACHE_ROOT="${DIMENET_CACHE_ROOT:-${PROJECT_DIR}/cached_tensors_dimenetpp_dielectric}"
RESULT_ROOT="${DIMENET_RESULT_ROOT:-${PROJECT_DIR}/results_dimenetpp_dielectric_fastfood}"
SUMMARY_CSV="${RESULT_ROOT}/dimenetpp_dielectric_fastfood_nb${NUM_BLOCKS}_summary.csv"
PRINT_PREFIX="[DIMENET-DIELECTRIC-nb${NUM_BLOCKS}]"

# dielectric sweep: dims 1/2/5/10/15/20/40/60/80/100% x 5 seeds (model:split = seed:seed+1000). 50 runs.
...
SEEDS=("123:1123" "456:1456" "789:1789" "234:1234" "567:1567")
RUNS=()
for d in "${DIMS[@]}"; do
...
run_one() {
  ...
  local cache_file="${cache_dir}/dimenetpp_dielectric_cached_tensors.pkl"
  if [[ ! -f "$cache_file" ]]; then
    echo "$PRINT_PREFIX ERROR: missing tensor cache ${cache_file}" >&2
    return 1
  fi

  local run_name="dimenetpp_dielectric_${WRAPPER}_nb${NUM_BLOCKS}_dim${id_dim_percent}_modelseed${model_seed}_splitseed${split_seed}_epochs${EPOCHS}"
  ...
  "$DIMENET_PYTHON" "${CODE_DIR}/dimenet_run_dielectric_v3.py" \
    --method "$WRAPPER" \
    --id_dim "$id_dim" \
