...
GPU="${DIMENET_GPU:-0}"
WRAPPER="fastfood"
CACHE_ROOT="${DIMENET_CACHE_ROOT:-${PROJECT_DIR}/cached_tensors_dimenetpp_bandgap}"
RESULT_ROOT="${DIMENET_RESULT_ROOT:-${PROJECT_DIR}/results_dimenetpp_bandgap_fastfood}"
SUMMARY_CSV="${RESULT_ROOT}/dimenetpp_bandgap_fastfood_summary.csv"
PRINT_PREFIX="[DIMENET-BANDGAP]"
...
if [[ ! -f "$SUMMARY_CSV" ]]; then
  echo "task,target_key,wrapper,id_dim,id_dim_percent,model_seed,split_seed,epochs,batch_size,cache_dir,output_dir,predictions_csv,history_json,val_mae,test_mae,duration_sec,status,exit_code,start_time,end_time" > "$SUMMARY_CSV"
fi
...
append_metadata() {
  ...
  "$DIMENET_PYTHON" - "$summary_csv" "$metadata_json" "$log_file" "$pred_csv" "$history_json" \
    ...
    "$output_dir" "$duration_sec" "$status" "$exit_code" "$start_time" "$end_time" "$write_summary" <<'PY'
...
(summary_csv, metadata_json, log_file, pred_csv, history_json, id_dim, id_dim_percent,
 ...
 exit_code, start_time, end_time, write_summary) = sys.argv[1:]
...
run_one() {
  ...
  local cache_file="${cache_dir}/dimenetpp_bandgap_cached_tensors.pkl"
  if [[ ! -f "$cache_file" ]]; then
    echo "$PRINT_PREFIX ERROR: missing tensor cache ${cache_file}" >&2
    return 1
  fi

  local run_name="dimenetpp_bandgap_${WRAPPER}_dim${id_dim_percent}_modelseed${model_seed}_splitseed${split_seed}_epochs${EPOCHS}"
  ...
  "$DIMENET_PYTHON" "${CODE_DIR}/dimenet_run_bandgap_v3.py" \
    --method "$WRAPPER" \
    --id_dim "$id_dim" \
    --epochs "$EPOCHS" \
  ...
  append_metadata "$SUMMARY_CSV" "$metadata_json" "$log_file" "$pred_csv" "$history_json" \
    "$id_dim" "$id_dim_percent" "$model_seed" "$split_seed" "$EPOCHS" "$BATCH_SIZE" "$cache_dir" \
    "$out_dir" "$duration_sec" "$status" "$exit_code" "$start_time" "$end_time" "$write_summary"

  echo "$PRINT_PREFIX DONE ${run_name} status=${status} exit_code=${exit_code} duration=${duration_sec}s"
}

SELECTED_RUNS="${DIMENET_RUNS:-${EFORM_RUNS:-}}"
