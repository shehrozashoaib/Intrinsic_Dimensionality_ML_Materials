...
# Required: DIMENET_TASK=eform|bandgap
# Required: DIMENET_SEEDS="s1 s2 ..." (split_seed = model_seed, matched convention)
set -uo pipefail
...
TASK="${DIMENET_TASK:?set DIMENET_TASK=eform|bandgap}"
...
GPU="${DIMENET_GPU:-0}"
WRAPPER="fastfood"
EPOCHS="${DIMENET_EPOCHS_OVERRIDE:-350}"
CACHE_ROOT="${PROJECT_DIR}/cached_tensors_dimenetpp_${TASK}"
CACHE_FILE_PFX="dimenetpp_${TASK}"
...
echo "[INFO] TASK=$TASK nb=$NUM_BLOCKS clipnorm=$CLIPNORM lr=$LR epochs=$EPOCHS seeds=$SEEDS"
...
target_key = "formation_energy_per_atom" if task == "eform" else "band_gap"
row = {
    "task": f"mp_{task}", "target_key": target_key, "wrapper": "fastfood",
