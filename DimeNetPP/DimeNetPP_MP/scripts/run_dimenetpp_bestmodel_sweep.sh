#!/usr/bin/env bash
# DimeNet++ Fastfood intrinsic-dimension sweep with BEST-MODEL evaluation, MP tasks (eform | bandgap).
#
...
#
# One job = one (task, dataset): full DIMS x SEEDS grid. Per-(task,dataset) summary CSV avoids
# races between concurrently running jobs.
#
# Env vars:
...
#   DIMENET_PATIENCE   default 100 (eform) / 75 (bandgap)   -- EarlyStopping on val_loss, 0=off
...
# --restore_best is ALWAYS passed. The MP runners have no --int_emb_size arg, so it is not passed.
#
# Usage:
#   DIMENET_TASK=eform   DIMENET_DATASET=full      bash scripts/run_dimenetpp_bestmodel_sweep.sh
#   DIMENET_TASK=bandgap DIMENET_DATASET=tenth_1   bash scripts/run_dimenetpp_bestmodel_sweep.sh
set -uo pipefail
...
if [[ -z "$TASK" || -z "$DATASET" ]]; then
  echo "ERROR: DIMENET_TASK (eform|bandgap) and DIMENET_DATASET (full|quarter_1|quarter_2|tenth_1|tenth_2) are required." >&2
  exit 1
fi
...
# Data-driven EarlyStopping patience (from plateau analysis of the previous sweeps).
if [[ "$TASK" == "eform" ]]; then
  DEFAULT_PATIENCE=100
else
  DEFAULT_PATIENCE=75
fi
PATIENCE="${DIMENET_PATIENCE:-$DEFAULT_PATIENCE}"

# Cache layout: FULL lives in cached_tensors_dimenetpp_<task>/, partitions in
# cached_tensors_dimenetpp_part_<task>_<dataset>/ -- both with the same pkl/scaler names.
# DIMENET_FIXEDTEST=1 selects the fixed-test 2-partition caches, where every partition of a
# seed is scored on that seed's FULL test set (built by build_fixedtest_partition_caches.py).
if [[ "$DATASET" == "full" ]]; then
  DEFAULT_CACHE_ROOT="${PROJECT_DIR}/cached_tensors_dimenetpp_${TASK}"
elif [[ "${DIMENET_FIXEDTEST:-0}" == "1" ]]; then
...
RESULT_ROOT="${DIMENET_RESULT_ROOT:-${PROJECT_DIR}/results_dimenetpp_bestmodel/${TASK}/${DATASET}}"
SUMMARY_CSV="${RESULT_ROOT}/dimenetpp_${TASK}_${DATASET}_bestmodel_summary.csv"
PRINT_PREFIX="[DIMENET-BEST-${TASK}-${DATASET}]"
...
# Optional override: DIMENET_DIMS="1.0:100 0.8:80" -> run only those dims. MUST sit OUTSIDE the
# if/else above -- it was previously nested in the `else`, so it silently never applied to eform.
...
(summary_csv, metadata_json, log_file, task, dataset, num_blocks, id_dim, id_dim_percent,
 ...
 write_summary, lr, lr_schedule) = sys.argv[1:]
...
run_one() {
  ...
  append_metadata "$SUMMARY_CSV" "$metadata_json" "$log_file" \
    "$TASK" "$DATASET" "$NUM_BLOCKS" "$id_dim" "$id_dim_percent" "$model_seed" "$split_seed" \
...
echo "$PRINT_PREFIX task=${TASK} dataset=${DATASET} num_blocks=${NUM_BLOCKS} epochs=${EPOCHS} patience=${PATIENCE} clipnorm=${CLIPNORM} restore_best=1 batch_size=${BATCH_SIZE} lr=${LR} lr_schedule=${LR_SCHEDULE} cache_root=${CACHE_ROOT}"
