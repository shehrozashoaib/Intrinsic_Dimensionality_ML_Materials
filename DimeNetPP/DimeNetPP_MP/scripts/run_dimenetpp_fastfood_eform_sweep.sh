...
RESULT_ROOT="${DIMENET_RESULT_ROOT:-${PROJECT_DIR}/results_dimenetpp_eform_fastfood}"
SUMMARY_CSV="${RESULT_ROOT}/dimenetpp_eform_fastfood_summary.csv"
PRINT_PREFIX="[DIMENET-EFORM]"
...
run_one() {
  ...
  local cache_file="${cache_dir}/dimenetpp_eform_cached_tensors.pkl"
  if [[ ! -f "$cache_file" ]]; then
    echo "$PRINT_PREFIX ERROR: missing tensor cache ${cache_file}" >&2
    return 1
  fi

  local run_name="dimenetpp_eform_${WRAPPER}_dim${id_dim_percent}_modelseed${model_seed}_splitseed${split_seed}_epochs${EPOCHS}"
  ...
  "$DIMENET_PYTHON" "${CODE_DIR}/dimenet_run_eform_v3.py" \
    --method "$WRAPPER" \
    --id_dim "$id_dim" \
    --epochs "$EPOCHS" \
