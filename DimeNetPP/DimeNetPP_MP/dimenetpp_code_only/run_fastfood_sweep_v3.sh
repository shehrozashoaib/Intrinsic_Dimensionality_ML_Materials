...
SEED=123
GPU=0
OUTDIR=./runs_dimenetpp_v3_fastfood
...
EOF
  ...
  LOGFILE="$LOGDIR/fastfood_dim${DIMPCT}pct_epochs${EPOCHS}_seed${SEED}.log"
  "$PYTHON_BIN" ./dimenet_run_v3.py     --method fastfood     --id_dim "$DIM"     --epochs "$EPOCHS"     --batch_size "$BATCH"     --seed "$SEED"     --gpu "$GPU"     --cache_dir "$CACHE_DIR"     --out_dir "$OUTDIR"     > "$LOGFILE" 2>&1
done
