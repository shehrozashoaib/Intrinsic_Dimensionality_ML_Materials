...
parser = argparse.ArgumentParser("DimeNet++ wrapper-v3 runner")
...
parser.add_argument("--orthonormal_backend", type=str, default="pytorch_cpu", choices=["tensorflow_cpu", "pytorch_cpu", "pytorch_gpu"], help="Backend for exact dense orthonormal projection")
...
parser.add_argument("--torch_q_cache_dir", type=str, default="./orthonormal_q_cache_v3", help="Cache directory for PyTorch-generated orthonormal Q matrices")
...
parser.add_argument("--out_dir", type=str, default="runs_dimenetpp_v3", help="Output directory for CSV/history")
parser.add_argument("--train_mode", type=str, default="wrapped", choices=["base", "wrapped"], help="Train base model or wrapper-v3 model")
args = parser.parse_args()

os.makedirs(args.out_dir, exist_ok=True)


def _maybe_precompute_orthonormal_q(args):
    if not (args.train_mode == "wrapped" and args.method == "dense" and args.orthonormal):
        return
    if args.orthonormal_backend not in ("pytorch_cpu", "pytorch_gpu"):
        return

    cache_dir = os.path.abspath(args.torch_q_cache_dir)
    os.makedirs(cache_dir, exist_ok=True)
    D = 83685
    if isinstance(args.id_dim, float):
        d = int(round(args.id_dim * D))
    else:
        d = int(args.id_dim)

    if args.orthonormal_backend == "pytorch_gpu":
        out_path = os.path.join(cache_dir, f"Q_rows{D}_cols{d}_seed{args.seed}_backend_pytorch_gpu{args.torch_q_gpu}.npy")
        device = "cuda"
    else:
        out_path = os.path.join(cache_dir, f"Q_rows{D}_cols{d}_seed{args.seed}_backend_pytorch_cpu.npy")
        device = "cpu"

    if os.path.exists(out_path):
        print(f"[DenseV3] Cached orthonormal Q already exists: {out_path}")
        return

    script_path = os.path.join(os.path.dirname(__file__), "torch_orthonormal_q_v3.py")
    cmd = [
        args.torch_python,
        script_path,
        "--rows", str(D),
        ...
        "--seed", str(args.seed),
        "--device", device,
        "--gpu", str(args.torch_q_gpu),
        "--out", out_path,
    ]
    print(f"[DenseV3] Precomputing orthonormal Q before TensorFlow init: {' '.join(cmd)}")
    subprocess.run(cmd, check=True)


_maybe_precompute_orthonormal_q(args)
...
if args.train_mode == "base":
    ...
    print("\nRunning WRAPPER-V3 DimeNet++ model\n")
    ...
    if args.method == "dense" and args.orthonormal:
        ...
        if args.orthonormal_backend in ("pytorch_cpu", "pytorch_gpu"):
...
if args.train_mode == "wrapped":
    print("Initialization check skipped in v3 runner to avoid Keras ragged/XLA predict issues on this stack.")
