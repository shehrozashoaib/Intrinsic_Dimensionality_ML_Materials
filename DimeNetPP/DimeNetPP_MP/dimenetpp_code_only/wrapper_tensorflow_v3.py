...
def _build_orthonormal_q_with_backend(rows: int, cols: int, seed: int, backend: str, torch_python: str, cache_dir: str, gpu: int):
    os.makedirs(cache_dir, exist_ok=True)
    if backend == "pytorch_gpu":
        out_path = os.path.join(cache_dir, f"Q_rows{rows}_cols{cols}_seed{seed}_backend_pytorch_gpu{gpu}.npy")
        device = "cuda"
    elif backend == "pytorch_cpu":
        out_path = os.path.join(cache_dir, f"Q_rows{rows}_cols{cols}_seed{seed}_backend_pytorch_cpu.npy")
        device = "cpu"
    else:
        raise ValueError(f"Unsupported backend for external orthonormal build: {backend}")
    ...
    if not os.path.exists(out_path):
        ...
        print(f"[DenseV3] Building orthonormal Q via {backend}: {' '.join(cmd)}")
        subprocess.run(cmd, check=True)
    else:
        print(f"[DenseV3] Reusing cached orthonormal Q ({backend}): {out_path}")
    return np.load(out_path)


class SubspaceProjectedGradTFV3(tf.keras.Model):
    """
    ...
    def __init__(
        ...
        orthonormal_backend: str = "pytorch_cpu",
        torch_python: str = "/venv/main/bin/python",
        torch_q_cache_dir: str = "/workspace/dimenet++/orthonormal_q_cache_v3",
        torch_q_gpu: int = 0,
        name: str = "SubspaceProjectedGradTFV3",
    ):
        ...
        if self.orthonormal_backend not in ("tensorflow_cpu", "pytorch_cpu", "pytorch_gpu"):
        ...
        if self.method == "dense":
            if (self.d == self.D) and self.full_rotation:
                ...
                if self.orthonormal_backend in ("pytorch_cpu", "pytorch_gpu"):
                    Q_np = _build_orthonormal_q_with_backend(
                        rows=self.D,
                        cols=self.d,
                        seed=self.seed,
                        backend=self.orthonormal_backend,
