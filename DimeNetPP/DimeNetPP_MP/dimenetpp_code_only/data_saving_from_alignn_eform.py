...
parser = argparse.ArgumentParser("Build DimeNet++ cached tensors from ALIGNN MP_json_eform/id_prop.json")
parser.add_argument("--id_prop", required=True, help="Path to ALIGNN-style id_prop.json.")
parser.add_argument("--id_key", type=str, default="material_id")
parser.add_argument("--target_key", type=str, default="formation_energy_per_atom")
...
parser.add_argument("--save_dir", type=str, default="./cached_tensors_dimenetpp_eform")
parser.add_argument("--tensor_filename", type=str, default="dimenetpp_eform_cached_tensors.npz")
parser.add_argument("--scaler_filename", type=str, default="scaler_eform.pkl")
parser.add_argument("--manifest_filename", type=str, default="manifest_eform.pkl")
...
def load_records(id_prop_path, id_key, target_key, limit=0, seed=DEFAULT_RANDOM_SEED):
    print(f"Reading {id_prop_path}")
    with open(id_prop_path, "r") as f:
        records = json.load(f)

    filtered = [r for r in records if r.get(target_key) is not None and r.get("atoms") is not None]
    random.seed(seed)
    ...
    targets = []
    ids = []
    for row in tqdm(filtered, desc="Converting JARVIS atoms to pymatgen structures"):
...
if len(structures) == 0:
    raise ValueError("No formation-energy structures were loaded. Nothing to save.")
print("Example:", ids[0], targets[0], structures[0])
...
manifest = {
    ...
    "source_id_prop": os.path.abspath(args.id_prop),
